...
function SignalPill({ signal }: { signal: 'buy' | 'neutral' | 'sell' }) {
  const c = SIG_COLOR[signal];
  return (
    <span style={{
...
function RSICard({ rsi }: { rsi: TechnicalAnalysis['rsi'] }) {
  ...
  return (
    <MiniCard>
      ...
      {v == null ? (
        <div style={{ color: '#64748b', fontSize: '0.82rem' }}>Insufficient data</div>
      ) : (
        <>
      ...
      {!macd ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
            {[
              { label: 'MACD Line', value: macd.macd, color: macd.trend === 'bullish' ? '#22c55e' : '#ef4444' },
              { label: 'Signal Line', value: macd.signal, color: '#94a3b8' },
              { label: 'Histogram', value: macd.histogram, color: macd.histogram >= 0 ? '#22c55e' : '#ef4444' },
            ].map(row => (
...
function BBCard({ bb }: { bb: TechnicalAnalysis['bollingerBands'] }) {
  if (!bb) return (
    <MiniCard>
      <SectionTitle>Bollinger Bands (20, 2)</SectionTitle>
      <div style={{ color: '#64748b', fontSize: '0.82rem' }}>Insufficient data</div>
    </MiniCard>
  );
  const pct = Math.min(Math.max(bb.percentB * 100, 0), 100);
  const posLabel = bb.signal === 'near_lower' ? 'Near Lower Band' : bb.signal === 'near_upper' ? 'Near Upper Band' : 'Mid Range';
  ...
  return (
    <MiniCard>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '4px', marginTop: '10px' }}>
        {[
          { label: 'Lower', value: bb.lower, color: '#22c55e' },
          { label: 'Middle', value: bb.middle, color: '#94a3b8' },
          { label: 'Upper', value: bb.upper, color: '#ef4444' },
        ].map(r => (
      ...
      <div style={{ marginTop: '8px', display: 'flex', justifyContent: 'center' }}>
        <span style={{ fontSize: '0.68rem', color: '#475569' }}>
          Width: {(bb.bandwidth * 100).toFixed(1)}%
        </span>
      </div>
    </MiniCard>
  );
    ...
    <MiniCard>
      ...
      {ma.cross && (
        <div style={{
          ...
          ✦ {ma.cross === 'golden' ? 'Golden Cross Detected' : 'Death Cross Detected'}
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{
                  ...
                  {ma.priceVsSma50 === 'above' ? '▲ Above' : '▼ Below'}
                </span>
              </div>
            </div>
          )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{
                  ...
                  {ma.priceVsSma200 === 'above' ? '▲ Above' : '▼ Below'}
                </span>
              </div>
            </div>
          )}
          {ma.sma50 != null && ma.sma200 != null && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '4px', borderTop: '1px solid #1e2d47' }}>
              <span style={{ fontSize: '0.78rem', color: '#64748b' }}>Alignment</span>
              <span style={{
                fontSize: '0.8rem', fontWeight: 700,
                color: ma.sma50VsSma200 === 'above' ? '#22c55e' : '#ef4444',
              }}>
                {ma.sma50VsSma200 === 'above' ? 'Bullish' : 'Bearish'}
              </span>
            </div>
          )}
        </div>
...
function SRCard({ sr, price }: { sr: TechnicalAnalysis['supportResistance']; price: number }) {
  ...
  return (
    <MiniCard>
      ...
      {empty ? (
        <div style={{ color: '#64748b', fontSize: '0.82rem' }}>No clear swing points detected</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          {[...sr.resistances].reverse().map((r, i) => (
            <div key={`r${i}`} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 10px', borderRadius: '5px', background: 'rgba(239,68,68,0.06)' }}>
              ...
              <span style={{ fontSize: '0.72rem', color: '#ef4444', minWidth: '60px' }}>Resistance</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 10px', borderRadius: '5px', background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.18)' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: '#3b82f6' }}>Current Price</span>
          </div>
          {sr.supports.map((s, i) => (
            <div key={`s${i}`} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 10px', borderRadius: '5px', background: 'rgba(34,197,94,0.06)' }}>
              ...
              <span style={{ fontSize: '0.72rem', color: '#22c55e', minWidth: '60px' }}>Support</span>
              <span style={{ fontSize: '0.72rem', color: '#64748b', minWidth: '48px', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
...
export default function AnalysisPanel({ symbol, price }: Props) {
  ...
  if (loading) {
    return (
      <div style={{ height: '280px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '14px', color: '#64748b' }}>
        ...
        <span style={{ fontSize: '0.82rem' }}>Computing technical indicators…</span>
      </div>
    );
  }
  ...
  const trend = TREND_LABEL[analysis.trend] ?? TREND_LABEL['neutral'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: '0.76rem', color: '#64748b', marginTop: '3px' }}>
            Trend: <span style={{ color: trend.color, fontWeight: 700 }}>{trend.label}</span>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '20px' }}>
          {([
            { label: 'Buy',     count: analysis.summary.buyCount,     color: '#22c55e' },
            { label: 'Neutral', count: analysis.summary.neutralCount, color: '#64748b' },
            { label: 'Sell',    count: analysis.summary.sellCount,    color: '#ef4444' },
          ] as const).map(item => (
      ...
      {/* Signal Table */}
      <div>
        <SectionTitle>Indicator Signals</SectionTitle>
        <div style={{ borderRadius: '8px', overflow: 'hidden', border: '1px solid #1a2d47' }}>
          {analysis.signals.map((sig, i) => (
            <div key={i} style={{
              ...
              <span style={{ flex: '0 0 140px', fontSize: '0.8rem', color: '#94a3b8' }}>{sig.name}</span>
              <SignalPill signal={sig.signal} />
              <span style={{ fontSize: '0.8rem', color: '#cbd5e1', flex: 1 }}>{sig.value}</span>
            </div>
          ))}
        </div>
      </div>
